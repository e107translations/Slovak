<?php
/*
+---------------------------------------------------------------+
|        e107 website content management system Slovak Language File
|        Released under the terms and conditions of the
|        GNU General Public License (http://gnu.org).
|        Last Modified: 2015/12/09 18:26:57
|
|        $Author: Jimako $
+---------------------------------------------------------------+
*/
define("ADMSLAN_6", "je hlavný administrátor a nemôže byť zmazaný.");
define("ADMSLAN_13", "Existujúci administrátori");
define("ADMSLAN_16", "Meno administrátora");
define("ADMSLAN_18", "Práva");
define("ADMSLAN_21", "Pridávanie administrátorov");
define("ADMSLAN_25", "Nahratie/manažovanie súborov");
define("ADMSLAN_27", "Dohliadanie na kategórie odkazov");
define("ADMSLAN_41", "Pridať/upraviť custom menu");
define("ADMSLAN_42", "Hodnotenia príspevku");
define("ADMSLAN_52", "Aktualizovať administrátora");
define("ADMSLAN_56", "Administrátor");
define("ADMSLAN_58", "Hlavný administrátor");
define("ADMSLAN_59", "Odobrať status administrátora");
define("ADMSLAN_61", "Administrátor zmazaný");
define("ADMSLAN_62", "Manažér pluginov");
define("ADMSLAN_71", "Kliknite sem pre zobrazenie oprávnení");
define("ADMSLAN_72", "ID admina: --ID-- meno: --NAME-- nové oprávnenia:");
define("ADMSLAN_73", "ID admina: --ID-- meno: --NAME--");


?>